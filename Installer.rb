...
def do_configs(configs, target, strip = 'conf/')
  ...
  if $operatingsystem == 'windows'
    ...
    # Mixin y acceso
    require 'win32/registry' # Require de un archivo
    include Win32::Registry::Constants # Incluye todos los metodos y variables
...
def do_libs(libs, strip = 'lib/')
  libs.each do |lf|
    next if File.directory? lf # next loop si <condicion>
...
def build_rdoc(files)
  return unless $haverdoc # return unless
  begin
    r = RDoc::RDoc.new
...
FileUtils.cd File.dirname(__FILE__) do
  ...
  do_libs(libs)
  do_locales(locales)
  do_man(man) unless $operatingsystem == "windows" # unless
end